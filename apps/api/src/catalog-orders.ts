...
export async function registerCatalogOrderRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/api/v1/branches/:branchId/menu', { preHandler: requireUser }, async (request, reply) => {
    ...
    const { openNow, timezone } = await branchOpenNow(branchId);
    const windows = await pool.query('SELECT product_id, day_of_week, time_from::text, time_to::text FROM branch_product_hours WHERE branch_id = $1', [branchId]);
    const windowsByProduct = new Map<string, { day_of_week: number; time_from: string; time_to: string }[]>();
    for (const window of windows.rows) {
      const list = windowsByProduct.get(window.product_id) ?? [];
      list.push(window);
      windowsByProduct.set(window.product_id, list);
    }
    const now = branchNow(timezone);
    const data = result.rows.map(product => {
      const list = windowsByProduct.get(product.id);
      const inWindow = !list || list.some(window => isWithinWindow(now, window.day_of_week, window.time_from, window.time_to));
      return { ...product, available_now: product.is_available && openNow && inWindow };
    });
    return { data, openNow };
  });
  ...
  app.post('/api/v1/orders/:id/items', { preHandler: requireUser }, async (request, reply) => {
    ...
    try { await client.query('BEGIN'); const order = await client.query('SELECT branch_id,company_id,status FROM orders WHERE id=$1 AND company_id=$2 FOR UPDATE', [orderId, (request.user as User).company_id]); if (order.rowCount !== 1 || order.rows[0].status !== 'DRAFT') { await client.query('ROLLBACK'); return reply.code(409).send({ error: 'El pedido no admite detalles' }); }
      ...
      const availability = await productAvailableNow(order.rows[0].branch_id, input.data.productId);
      if (!availability.available) { await client.query('ROLLBACK'); return reply.code(409).send({ error: availability.openNow ? 'Producto no disponible en este horario' : 'Sucursal cerrada en este horario' }); }
